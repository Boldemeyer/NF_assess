...
Within each life-stage, the plots are essentially showing the same thing. Consider consolidating to one figure for each life-stage that covers both Chinook salmon and steelhead.
...
Chinook redds, steelhead redds, and steelhead summer parr limiting life stages. Improve low capacity habitat for redds further upstream, juveniles habitat below. Improve connectivity. Add details and discuss further.
...
# Need to break up the dataframe for labeling purposes
NF_geo_rch_label_1 = NF_geo_reach_chnk %>%
  filter(GNIS_Name =="North Fork Salmon River")
NF_geo_rch_label_2 = NF_geo_reach_chnk %>%
  filter(!GNIS_Name == "North Fork Salmon River")
...
chnk_sum_georch = NF_geo_reach_chnk %>%
  ...
  scale_color_viridis_c(direction = -1) +
    geom_label_repel(
    aes(label = Reach, geometry = geometry),
    stat = "sf_coordinates",
  ...
  theme(legend.position = "right")
        ...
        #axis.ticks = element_blank()) +
  labs(title = "Chinook, Summer Juveniles",
       color = expression(`Summer Juv` / km))
...
chnk_win_georch = NF_geo_reach_chnk %>%
  ...
  theme(legend.position = "bottom",
        ...
        axis.ticks = element_blank()) +
  labs(title = "Chinook, Winter Juveniles",
       color = expression(`Winter Juv` / km))
...
chnk_sum_cap_h12 %>%
  ...
  mutate(tot_length = tot_length / 1000,
         'Avg. Capacity/km' = tot_cap/tot_length,
         'HUC12 Name' = replace_na(`HUC12 Name`, 'Total')) %>%
